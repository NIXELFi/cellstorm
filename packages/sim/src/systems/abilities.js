import { GRID_SIZE, makeCell } from "../world";
 ...
 */
export function abilitiesSystem(w, sink) {
    for (let i = 0; i < w.cells.length; i++) {
        const c = w.cells[i];
        if (!c.alive || c.stunT > 0)
            continue;
        ...
        if (c.plagueT > 0) {
            ...
            c.hp -= 0.07;
            if (c.hp <= 0) {
                handleDeath(w, sink, c, null);
                continue;
            }
        }
        if (p.regen && c.hp < c.maxHp)
            c.hp = Math.min(c.maxHp, c.hp + p.regen);
        if (p.shoot) {
            if (c.cdShoot > 0)
                c.cdShoot--;
            else if (c.tx !== null) {
